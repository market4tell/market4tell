...
if (TFCache::start_cache($args['mod_name'], self::$post_id, array('ID' => $args['module_ID']))):
    ...
    $container_class = apply_filters('themify_builder_module_classes', array(
        'module', 'module-' . $args['mod_name'], $args['module_ID'], $chart_class,'layout-' . $fields_args['layout_feature'], 'size-' . $fields_args['circle_size_feature'], $fields_args['css_feature'], $animation_effect
   ...
   <div <?php echo self::get_element_attributes(self::sticky_element_props($container_props,$fields_args)); ?>>
        ...
        if ($fields_args['circle_stroke_feature']) {
            ...
            $style.=".{$args['module_ID']} .module-feature-chart-html5 {
			    box-shadow: inset 0 0 0 " . $fields_args['circle_stroke_feature'] . "px {$circleBackground};
			}
			.{$args['module_ID']} .chart-loaded.chart-html5-fill {
			    box-shadow: inset 0 0 0 " . $fields_args['circle_stroke_feature'] . "px {$circleColor};
		    }";
        }
        if ($insetColor !== '') {
            $style.=".{$args['module_ID']} .chart-html5-inset {background-color: {$insetColor}}";
        }
        $style.='</style>';
        echo $style;
        ?>
