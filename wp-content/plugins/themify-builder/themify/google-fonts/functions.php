...
function themify_enqueue_google_fonts() {
	...
	$fonts = array();
	foreach ( apply_filters( 'themify_google_fonts', array() ) as $key => $font ) {
		if ( ! empty( $font ) && preg_match( '/^\w/', $font ) ) {
			...
			$fonts[ $key ] = preg_replace( '/,/', ':', str_replace( ':', ',', $font ), 1 );
		}
	}

	if ( ! empty( $fonts ) ) {
		$path = ( is_ssl() ? 'https' : 'http' ) . '://fonts.googleapis.com/css?family=' . implode( '|', $fonts );
