        ...
        Lightbox: {
                ...
                var self = ThemifyBuilderCommon.Lightbox,
                    ...
                    callback = function (response) {
                            ...
                            if ('inline' !== options.loadMethod){
                                ...
                                for(var i=tabs.length-1;i>-1;--i){
                                    ...
                                    for(var j=a.length-1;j>-1;--j){
                                         a[j].addEventListener('click', tb_app.Constructor.switchTabs);
                                    }
                                }
                                tabs =a= null;
                            }
                            tb_app.undoManager.disable();
                            if(tb_app.mode==='visual'){
                                $(document).triggerHandler('mouseup');
                            }
                        ...
                        if ('inline' === options.loadMethod) {
                            ...
                            callback(tb_app.Constructor.run(options.templateID));
                        }
                        else if (options.loadMethod === 'html') {
                            tb_app.autoSaveCid = null;
                            self.close(true);
                            if(options.contructor===true){
                                callback(tb_app.Constructor.run(options.data,options.save));
                            }
                            else{
                           ...
                           $.ajax(options)
                            .done(function (response) {
                                if(options.contructor===true){
                                    callback(tb_app.Constructor.run(response));
                                }
                                else{
                                    callback(response);
                                }
            ...
            controlByKeyInput: function () {
                function save(e) {
                    ...
                    else if (83 === e.which && document.activeElement.tagName !== 'INPUT' && document.activeElement.tagName !== 'TEXTAREA' && (true === e.ctrlKey || true === e.metaKey)) {
                        e.preventDefault();
                        e.stopPropagation();
                ...
                $(document).on('keydown.tb_shortcut', save);
                if (tb_app.mode === 'visual') {
                    $(topWindow.document).on('keydown.tb_shortcut', save);
                }
            },
            responsiveTabs: function () {
                    var tabs=null,ul,tabsWidth,
                        ...
                        callback = function (e, id, container) {
                            setTimeout(function(){
                                if(tb_app.Constructor.clicked==='styling'){
