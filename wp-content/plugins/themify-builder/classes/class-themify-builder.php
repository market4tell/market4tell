...
if (!class_exists('Themify_Builder')) :
	...
	public function init() {
	    ...
	    add_action('wp_ajax_nopriv_tb_save_css', array($this, 'save_builder_css'), 10);
	    // Script Loader
	...
	public function setup() {
		...
		if (!$is_action || !in_array($action, array('tb_update_tick', 'tb_load_module_partial', 'tb_render_element', 'tb_load_shortcode_preview', 'themify_builder_plupload_action', 'tb_module_favorite', 'themify_regenerate_css_files_ajax', 'render_element_shortcode_ajaxify', 'themify_get_tax', 'themify_get_post_types', 'tb_get_menu', 'themify_builder_get_tax_data', 'tb_help'), true)) {
	...
	public function defer_js_admin($tag, $handle, $src) {
	    if (in_array($handle, self::$async, true)) {
		return str_replace(' src', ' async="true" src', $tag);
	    }
	    return $tag;
	...
	public function save_builder_css() {
	    ...
	    if (!empty($data) && is_array($data)) {
		echo json_encode(Themify_Builder_Stylesheet::write_stylesheet((int) $_POST['id'], $data));
	    }
	    die;
	}
	...
	public function builder_show_on_front($content) {
	    ...
	    if ((!$this->skip_display_check ) && (
		    !is_object($post) || ( is_admin() && !defined('DOING_AJAX') ) || (!Themify_Builder_Model::is_front_builder_activate() && false === apply_filters('themify_builder_display', true, $post_id) ) || post_password_required() || ( themify_is_woocommerce_active() && ( ( is_shop() ) || ( is_singular('product') && 'product' === get_post_type() ) ) )
		    )) {
		return $content;
	    }
	    ...
	    $module_list = Themify_Builder_Model::isWpEditorDisable() === true ? $this->get_flat_modules_list($post_id, null, true) : null;

	    if (!$this->in_the_loop && Themify_Builder_Model::is_front_builder_activate()) {
		if (!empty($module_list)) {
		    $content = '';
		}
		return $this->get_active_builder_data($post_id, $content) . $this->get_builder_stylesheet('', $post_id);
	    }
	    ...
	    return $this->get_builder_stylesheet($builder_output, $post_id) . $content;
	}
	...
	public function get_builder_stylesheet($builder_output, $post_id = false) {
		...
		return '<script type="text/javascript">
				if(document.getElementById( "themify-builder-style" )===null ){
					...
					link.rel="stylesheet";
					link.type="text/css";
					link.href="' . themify_enque(THEMIFY_BUILDER_URI . '/css/themify-builder-style.css') . '?ver=' . THEMIFY_VERSION . '";
	...
	public function load_javascript_template_admin() {
	    add_filter('script_loader_tag', array($this, 'defer_js_admin'), 11, 3);
	...
	public function load_form_templates() {
	    check_ajax_referer('tb_load_nonce', 'tb_load_nonce');
	    echo json_encode(self::getComponentJson(false));
	    wp_die();
	}
	...
	function find_text_inputs_in_module($options) {
		...
		} else if (isset($option['type']) && in_array($option['type'], $allowed_types)) {
		    $inputs[] = $option['id'];
		}
	    }
	    return $inputs;
	...
	public function themify_find_and_replace_ajax() {
		...
		if ($data !== $new_data) {
		    ...
		    $new_data = json_decode(stripslashes_deep($new_data), true);
		    if (is_array($new_data)) {
			$ThemifyBuilder_Data_Manager->save_data($new_data, $post_id);
		    }
		}
	    }
	    global $replacementsCount;
