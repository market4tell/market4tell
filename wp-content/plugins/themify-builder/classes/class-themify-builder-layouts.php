...
class Themify_Builder_Layouts {
	...
	public function custom_meta_boxes( $post ) {
		/*
		 * Disable all existing meta boxes, then re-add 'submitdiv', 'postimagediv', and Themify Custom Panel
		 */
		global $wp_meta_boxes;
		unset( $wp_meta_boxes[ $this->layout->post_type_name ], $wp_meta_boxes[ $this->layout_part->post_type_name ] );
		add_meta_box( 'submitdiv', __( 'Publish', 'themify' ), 'post_submit_meta_box', array( $this->layout->post_type_name, $this->layout_part->post_type_name ), 'side', 'default' );
		add_meta_box( 'postimagediv', __( 'Featured Image', 'themify' ), 'post_thumbnail_meta_box', array( $this->layout->post_type_name, $this->layout_part->post_type_name ), 'side', 'default' );
		add_meta_box( 'themify-meta-boxes', __( 'Themify Custom Panel', 'themify' ), array( Themify_Metabox::get_instance(), 'render' ), array( $this->layout->post_type_name, $this->layout_part->post_type_name ), 'normal', 'default' );
	...
	public function get_list_output() {
		...
		<div class="tb_layout_sticky">
			<div class="tb-layout-search-container">
                <input type="text" placeholder="<?php _e( 'Search', 'themify' ); ?>" data-search="true" autofocus id="tb_layout_search" required />
